...
- [x] **9.4** Create `frontend/app/(app)/layout.tsx` — auth check (redirect to /login if unauthenticated), renders AppShell, calls fetchKeys() and loads debate list on mount
- [x] **9.5** Create `frontend/app/(app)/page.tsx` — redirect to /app/debates or /app/new
...
- [x] **10.2** Create `frontend/components/debate/CreateDebateForm.tsx` — topic textarea, two AgentConfig panels (blue/green accents), max turns input, submit flow (validate, check hasKey, call createDebate, navigate to debate view, start loop)
- [x] **10.3** Create `frontend/app/(app)/new/page.tsx` — renders CreateDebateForm
...
- [x] **11.2** Create `frontend/components/debate/DebateMessage.tsx` — single message bubble (agent name + model in header, turn number, markdown content via react-markdown, blue-50 for agent A, emerald-50 for agent B)
- [x] **11.3** Create `frontend/components/debate/DebateStatus.tsx` — status badge/header (topic, running/paused/completed indicator, turn counter, agent info)
- [x] **11.4** Create `frontend/components/debate/DebateView.tsx` — message list with auto-scroll (pause on manual scroll up, resume at bottom), Pause button, Resume button for paused, Share button for completed, error display
- [x] **11.5** Create `frontend/app/(app)/debate/[id]/page.tsx` — load debate from DebateManager or API, auto-start if status=created, show Resume for paused/running
...
- [x] **12.2** Create `frontend/app/(app)/settings/page.tsx` — renders ApiKeyForm for OpenAI and Anthropic
...
- [x] **13.3** Create `frontend/app/shared/[id]/page.tsx` — public shared debate view (no auth, fetch via getSharedDebate, reuse DebateMessage, SEO metadata from debate topic, 404 for non-completed, CTA to landing page)
...
- [x] **14.2** Create `frontend/Dockerfile` — node:18-alpine, multi-stage build (install → build → production), standalone output, CMD node server.js, expose 3000
